(function ($) {
	$.fn.zepto_china_area = function () {
		...
		this.getAreaValue = function () {
			...
			var result = $(select[0]).select2('data').text + '-' + $(select[1]).select2('data').text + '-' + $(select[2]).select2('data').text
			return result;
		}
		...
		return this.each(function () {
			...
			//设置默认标题
			$(objSelect[0]).append(title[0]);
			$(objSelect[1]).append(title[1]);
			$(objSelect[2]).append(title[2]);
			//设置select样式，增加index以确保唯一
			$(objSelect[0]).addClass('loc_province'+index);
			$(objSelect[1]).addClass('loc_city'+index);
			$(objSelect[2]).addClass('loc_town'+index);
			//调用select2插件
			$(objSelect[0]).select2();
			$(objSelect[1]).select2();
			$(objSelect[2]).select2();
			//选择框内容发生变化时，调用location方法实现二三级选择框内容联动
			$(objSelect[0]).change(function () {
				//获取当前选择框所在区域的index
				var cur = $(this).closest('.china-area').index();
				$(objSelect[1]).empty();
				$(objSelect[1]).append(title[1]);
				//增加index确保是为当前选择框变更联动内容
				loc.fillOption('loc_city'+cur, '0,' + $(objSelect[0]).val());
				$(objSelect[1]).change();
			})
			
			$(objSelect[1]).change(function () {
				var cur = $(this).closest('.china-area').index();
				$(objSelect[2]).empty();
				$(objSelect[2]).append(title[2]);
				loc.fillOption('loc_town'+cur,'0,' + $(objSelect[0]).val() + ',' + $(objSelect[1]).val());
			})
			loc.fillOption('loc_province'+index, '0');
		})	
	}
})(Zepto);
